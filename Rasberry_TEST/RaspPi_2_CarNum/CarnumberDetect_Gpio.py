...
# GPIO 핀 설정
IR_SENSOR = 4                                               # IR센서 GPIO핀 4번 사용
CAMERA_INDEX = 0                                            # 카메라 사용시 카메라 장치 인덱스 0번으로 설정
# 센서 초기화
GPIO.setmode(GPIO.BCM)                                      # GPIO핀 모드 : BCM모드로 설정
GPIO.setup(IR_SENSOR, GPIO.IN)                              # IR센서 입력으로 설정
camera = cv2.VideoCapture(CAMERA_INDEX)                     # 카메라 객체 초기화 / OpenCV의 VideoCapture 클래스를 사용하여 카메라 연결 / CAMERA_INDEX로 설정된 인덱스의 카메라 열기
img_path = ""                                               # 이미지 경로 초기화

IR_Sensor_data = GPIO.input(IR_SENSOR)                      # IR 센서 값 출력을 위해서 .. 보기 좋게 하려고 
car_Number_Fin = "_"                                       # MQTT 통신에 사용하기 위해 전역 변수 선언   
...
def car_num_detect(img_path):
    ...
    global car_Number_Fin           # 최종 출력되는 번호판 번호를 전역번수에 대입  
    car_Number_Fin = chars           # 최종 출력되는 번호판 번호를 전역번수에 대입  

    # print(type(chars))             # 전역 변수에 들어가는 번호판 dtype 확인 -> str 확인해서 생략  
...
def IR_Func():
    try:
        global IR_Sensor_data
        IR_Sensor_data = GPIO.input(IR_SENSOR)

        if IR_Sensor_data == GPIO.LOW:
            sleep(1)
            ...
            car_Number_Fin = car_num_detect(img_path)            
    except:
        pass